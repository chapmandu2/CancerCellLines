...
head(dietlein_data)
getDrugData_custom(dietlein_data, drugs = 'KU60648', cell_lines = c('DMS114_LUNG', 'A549_LUNG'))
...
```{r eval=FALSE}
    ...
    random_genes <- dplyr_con %>% tbl('ccle_affy') %>% group_by(Symbol) %>% summarise(N=n()) %>% 
      ungroup() %>% collect %>% 
      dplyr::filter(N < mean(N)) %>% sample_n(200) %>% as.data.frame
    ...
    #without resp data
    big_df <- make_df(full_con, random_genes, random_cell_lines, drugs=random_drugs, data_types=c('affy', 'cn', 'hybcap', 'cosmicclp'))
    
    #with custom resp data
    big_df <- make_df(full_con, random_genes, cell_lines = c('DMS114_LUNG', 'A549_LUNG'), drugs = 'KU60648', drug_df = dietlein_data)
