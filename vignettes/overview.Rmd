...
The `make_tall_df` function does this merging in a standard way and returns the the data in a 'tidy' format that is useful for plotting in ggplot2 or further manipulation with tidyr.  

```{r}
make_tall_df(test_db, symbols, cell_lines, drugs)
```

The `make_wide_df` function can take the output from `make_tall_df` and create a wide or matrix-like data frame which is a conveninent input for modelling packages such as caret.

```{r}
my_df <- make_tall_df(test_db, symbols, cell_lines, drugs)
make_wide_df(my_df)

```

Finally, there is the `make_df` function which generates a wide data frame directly.
...
```{r}
make_df(test_db, symbols, cell_lines, drugs, data_types=c('hybcap', 'affy', 'resp'))
make_df(test_db, symbols, 
...
```{r eval=FALSE}
    ...
    big_df <- make_df(full_con, random_genes, random_cell_lines, random_drugs)
    
    #without resp data
    big_df <- make_df(full_con, random_genes, random_cell_lines, drugs=NULL, data_types=c('affy', 'cn', 'hybcap', 'cosmicclp'))
    
    #with custom resp data
    big_df <- make_df(full_con, random_genes, cell_lines = c('DMS114_LUNG', 'A549_LUNG'), drugs = 'KU60648_pGI50', drug_df = dietlein_data)
