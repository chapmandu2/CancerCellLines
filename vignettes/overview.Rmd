...
Define where the data is to be stored/found.  Files downloaded from the [CCLE project website](http://www.broadinstitute.org/ccle/data/browseData?conversationPropagation=begin).  
...
```{r eval=FALSE}
    ...
    random_genes <- dplyr_con %>% tbl('ccle_affy') %>% dplyr::select(Symbol) %>%
      distinct %>% collect %>% sample_n(2000) %>% as.data.frame
...
This should take no more than 1-2 seconds for either constituent retrieval, and ~5 seconds to make the data frame depending on your hardware.
