...
importCCLE_cn <- function ( fn , con ) {
  ...
  data_tall <- reshape2::melt(data, id.vars =  colnames(data)[1:4])
  colnames(data_tall) <- c('Symbol', 'Chr', 'txStart', 'txEnd', 'CCLE_name', 'log2cn')

  #write to db
  message('Writing to database')
  DBI::dbWriteTable(con, "ccle_cn", data_tall, overwrite=TRUE)

  ##index
  message('Indexing the table')
  DBI::dbSendQuery ( con , sprintf(' CREATE INDEX `ccle_cn_symbol` ON `%s` (`Symbol` ASC); ', 'ccle_cn' )   )
  DBI::dbSendQuery ( con , sprintf(' CREATE INDEX `ccle_cn_CCLE_name` ON `%s` (`CCLE_name` ASC); ', 'ccle_cn' )   )
  DBI::dbSendQuery ( con , sprintf(' CREATE INDEX `ccle_cn_symbol_AND_CCLE_name` ON `%s` (`Symbol`,`CCLE_name` ASC); ', 'ccle_cn' )   )
