...
#' @export
make_tall_df <- function(con, genes, cell_lines, drugs, data_types=c('affy', 'cn', 'hybcap', 'resp', 'cosmicclp'), drug_df=NULL) {
...
make_df <- function(con, genes, cell_lines, drugs, data_types=c('affy', 'cn', 'hybcap', 'resp', 'cosmicclp'), drug_df=NULL) {
  ...
  #run make_tall_df function
  all_data <- make_tall_df(con, genes, cell_lines, drugs, data_types, drug_df)

  #create matrix
  output.df <- all_data %>% make_wide_df

  return(output.df)

}

#' Turn a tall data frame from make_tall_df into a wide data frame
#'
#' This function creates a \code{data.frame} containing data for different data types in a form suitable for further statistical modelling in R.
#'
#' @param df A \code{data.frame} in tall format generated by \code{make_tall_df}
#' @return A \code{data.frame} in wide format
#' @export
make_wide_df <- function(df) {
