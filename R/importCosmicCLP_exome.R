...
importCosmicCLP_exome <- function ( fn , con ) {
  ...
  DBI::dbWriteTable(con, "cosmicclp_exome", as.data.frame(data), overwrite=TRUE)

  ##index
  message('Indexing the table')
  DBI::dbSendQuery ( con , sprintf(' CREATE INDEX `cosmicclp_exome_gene_name` ON `%s` (`gene_name` ASC); ', 'cosmicclp_exome' )   )
  DBI::dbSendQuery ( con , sprintf(' CREATE INDEX `cosmicclp_exome_sample_name` ON `%s` (`sample_name` ASC); ', 'cosmicclp_exome' )   )
  DBI::dbSendQuery ( con , sprintf(' CREATE INDEX `cosmicclp_exome_gene_name_AND_sample_name` ON `%s` (`gene_name`,`sample_name` ASC); ', 'cosmicclp_exome' )   )
