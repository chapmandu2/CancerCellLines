...
#' @export
plotRespVsGeneticHist <- function(df, data_type=NULL, facet_option=FALSE, label_option=FALSE) {

  plot_data <- df %>% filter(!is.na(resp_value)) %>% arrange(desc(resp_value))

  if(data_type %in% c('hybcap', 'cosmicclp')) {
    p <- ggplot(plot_data, aes(x=native_id, y=resp_value, fill=as.factor(feature_value)) ) +
      geom_bar(stat='identity') +
      scale_x_discrete(limits=plot_data$native_id) +
      ylab(unique(plot_data$resp_id)) +
      theme_bw() + theme(axis.text.x = element_text(size=rel(1), angle=330, hjust=0, vjust=1))

  } else {
    p <- ggplot(plot_data, aes(x=native_id, y=resp_value, fill=scale(feature_value)) ) +
      geom_bar(stat='identity') +
      scale_x_discrete(limits=plot_data$native_id) +
